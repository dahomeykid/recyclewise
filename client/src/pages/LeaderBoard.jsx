...
const LeaderBoard = () => {
	...
	const { user, API_URL } = useRecycleWise(); 
	...
	useEffect(() => {
		const getList = async () => {
			try {
				const response = await axios.get(`${API_URL}/leaderboard`); // Fetch leaderboard data from the API
					...
					<div className='mt-4 space-y-3'>
						{users.map((elt, index) => (
							...
							>
								...
								<div className='col-span-6 text-gray-900'>{elt.userId.email}</div>
